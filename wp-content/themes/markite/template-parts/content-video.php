...
$markite_video_url = function_exists('get_field') ? get_field( 'fromate_style' ) : NULL;

if( is_single() ): ?>
    <article id="post-<?php the_ID(); ?>" <?php post_class('postbox__item-details format-video fix mb-70'); ?>>
        <?php if( has_post_thumbnail() ): ?>
           <div class="postbox__thumb postbox__video">
              <a href="<?php the_permalink(); ?>" class="w-img">
                 <?php the_post_thumbnail('full', array('class' => 'img-responsive' )); ?>
              </a>
              <div class="postbox__play">
                 <a href="<?php print esc_url($markite_video_url); ?>" data-fancybox ><i class="fas fa-play"></i></a>
              </div>
           </div>
        <?php endif; ?>
...
<?php
    ...
    <article id="post-<?php the_ID(); ?>" <?php post_class('postbox__item format-video fix mb-50'); ?>>
        <?php if( has_post_thumbnail() ): ?>
           <div class="postbox__thumb postbox__video">
              <a href="<?php the_permalink(); ?>" class="w-img">
                 <?php the_post_thumbnail('full', array('class' => 'img-responsive' )); ?>
