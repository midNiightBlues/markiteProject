...
$store_open_notice        = isset( $store_info['dokan_store_open_notice'] ) && ! empty( $store_info['dokan_store_open_notice'] ) ? $store_info['dokan_store_open_notice'] : __( 'Store Open', 'markite' );
$store_closed_notice      = isset( $store_info['dokan_store_close_notice'] ) && ! empty( $store_info['dokan_store_close_notice'] ) ? $store_info['dokan_store_close_notice'] : __( 'Store Closed', 'markite' );
...
if( !empty($_REQUEST['vendor_id']) ) {
    $to = !empty($store_user->data->data->user_email) ? $store_user->data->data->user_email : get_option( 'admin_email' );
    $subject = !empty($_REQUEST['subject']) ? $_REQUEST['subject'] : 'Vendor subject';
    
    $email = !empty($_REQUEST['email']) ? $_REQUEST['email'] : 'example@gmail.com';

    $message = "Hi there!<br><br>";
    $message .= "Contact Email ID: {$email}<br><br>Message:<br>";
    ...
    //$headers .= "Reply-to: <$to>\r\n";
    markite_wp_mail( $to, $subject, $message, $headers );
    ?>
    <script type="text/javascript">
        window.location.href = "<?php echo esc_html($request_url); ?>?status=sent";
    </script>
    <?php
}

$_status = !empty($_GET['status']) ? $_GET['status'] : '';
            ...
            <div class="profile-info-summery-wrapper dokan-clearfix">
                <div class="profile-info-summery">
                    ...
                    <div class="profile-info">
                        ...
                        <div class="markite-vendor-form">
                            <h4>Inguery</h4>
                            <?php if ( $_status == 'sent' ): ?>
                                <p>sent</p>
                            <?php endif; ?>
                            <form id="markite-vendor-mail-form" method="post" action="<?php echo esc_html($request_url); ?>">
                                <input type="hidden" name="vendor_id" value="<?php echo esc_html($store_user->get_id()); ?>">
