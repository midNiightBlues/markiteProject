...
<?php if ( $r->have_posts() ) : ?>
    ...
    <?php while ( $r->have_posts() ): $r->the_post() ?>
        ...
        <li>
            ...
            <?php else: ?>
                <?php if ( ! empty( $show_rating ) ) echo markite_dokan_product_rating($product);  // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped ?>
            <?php endif ?>

            <?php echo markite_dokan_product_price($product);  // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped ?>
        </li>
        <?php endwhile; ?>
    </ul>
<?php else: ?>
    <p><?php esc_html_e( 'No products found', 'markite' ); ?></p>
<?php endif; ?>
